...
{
    static string 
        RE_EMAIL = @"(?<=(^|\s))[a-z0-9-]+@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?(?=($|\s))",
        RE_SURNAME_NAME = @"\b\p{Lu}\p{Ll}+\s\p{Lu}\p{Ll}+\b",
        RE_SURNAME_INIT = @"\b\p{Lu}\p{Ll}+\s\p{Lu}\.\p{Lu}\.(?=($|\s))",
        RE_DOMAINS_UA = @"(?<=(^|\s))(?i)([a-z]+[a-z0-9-]*[a-z0-9]\.){1,60}(com|net|org|edu|in)\.ua\b",
        ...
        RE_POSTAL_ADDRESS = @"(?<=(^|\s))(\p{Lu}\p{Ll}+)((\s+\p{Lu}\p{Ll}+){1,2}|(\s+\p{Lu}\.\s*\p{Lu}\.))\s*\,((\w|\-|\s|\d|\.|/)+,){2,6}\s*[0-9]{5}(?=($|\s))",
